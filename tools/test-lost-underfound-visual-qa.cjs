...
assert.strictEqual(bottlecapBounds.length, 5, "Bottlecap idle must exclude truncated source frames");
const widths = bottlecapBounds.map((bounds) => bounds.w);
assert.ok(Math.max(...widths) / Math.min(...widths) < 1.3, "Bottlecap idle frames must not contain crop-scale outliers");
