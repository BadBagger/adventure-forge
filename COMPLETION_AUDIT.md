...
```powershell
node --check app.js
node --check tools\build-playable.cjs
node tools\build-playable.cjs adventureforge-playable-fixture.json adventureforge-pilot.playable.html
```
...
The current implementation satisfies the requested UI/tooling objective as a working MVP. Remaining work is product-hardening and expansion rather than a missing requested core surface:
...
- More formal automated browser tests outside the current smoke artifacts.
